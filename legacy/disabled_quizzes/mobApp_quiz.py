...
# تحميل جميع أسئلة تطبيقات موبايل من GitHub بدون تحديد عدد أو تصفيتها
def load_mobApp_quiz():
    url = "https://raw.githubusercontent.com/AboALhasanx/json-files/refs/heads/main/mobApp_quiz.json"
...
def handle_mobApp_quiz_menu(bot, message):
    quiz_menu = ReplyKeyboardMarkup([ 
        [KeyboardButton("▶️ بدء الاختبار (تطبيقات موبايل)")],
        [KeyboardButton("🎲 سؤال عشوائي (تطبيقات موبايل)")],
...
def send_mobApp_question(bot, chat_id, context, index):
    quiz_data = context["mobApp_quiz"]
    ...
    if index < total_questions:
        ...
        if index + 1 < total_questions:
            keyboard = InlineKeyboardMarkup([ 
                [InlineKeyboardButton(f"➡️ التالي", callback_data="next_mobApp")]
            ])
...
def start_mobApp_test(bot, message):
    chat_id = message.chat.id
    quiz = load_mobApp_quiz()
    if not quiz:
    ...
    send_mobApp_question(bot, chat_id, user_data[chat_id], 0)

# زر التالي في الاختبار
def next_mobApp_handler(bot, call):
    chat_id = call.message.chat.id
    if chat_id in user_data:
        user_data[chat_id]["index"] += 1
        send_mobApp_question(bot, chat_id, user_data[chat_id], user_data[chat_id]["index"])
    bot.answer_callback_query(call.id)

# سؤال عشوائي بدون تتبع
def random_mobApp_question(bot, message):
    quiz = load_mobApp_quiz()
    if not quiz:
